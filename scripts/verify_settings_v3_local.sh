...
SV3_JS="${2:-settings_v3/settings_v3.js}"
if [[ -f "$SV3_JS" ]]; then
  echo "=== settings_v3 module ($SV3_JS) ==="
  echo "MD5: $(md5 -q "$SV3_JS" 2>/dev/null || md5sum "$SV3_JS" | awk '{print $1}')"
  echo
  FILE="$SV3_JS"
  check "My Profiles (module)" 'My Profiles'
  check "Table orbs (module)" 'rm-sv3-oa-table'
  check "Advanced Orbs label (module)" 'Advanced Orbs'
  check "Chiron above fold (module)" 'chiron.*Chiron'
  check "collectSettingsV3Patch (module)" 'collectSettingsV3Patch'
  FILE="${1:-app_shell.html}"
fi
