...
Verifies:
  * Sandbox loads shared genie_variable_builder module over HTTP (no search backend)
...
def ensure_server() -> tuple[str, subprocess.Popen | None]:
    ...
    def assets_available(base: str) -> bool:
        try:
            with urllib.request.urlopen(f"{base}/genie_variable_builder.js", timeout=2) as resp:
                if resp.status != 200:
                    return False
            with urllib.request.urlopen(f"{base}/genie_SANDBOX_variable_builder.html", timeout=2) as resp:
    ...
    try:
        with urllib.request.urlopen(f"{BASE}/health", timeout=2) as resp:
            if resp.status != 200 or not assets_available(BASE):
        ...
        if not wait_server(base) or not assets_available(base):
            proc.terminate()
            fail(f"Could not start temp server with genie sandbox on {base}")
        return base, proc
    return BASE, proc
...
def main() -> int:
    ...
    url = f"{base}/genie_SANDBOX_variable_builder.html"
    ...
    try:
        with sync_playwright() as pw:
            ...
            # No search backend HTTP (static module assets allowed)
            http_reqs: list[str] = []
            ...
            page.wait_for_selector("#variableCards .variable-card")
            search_calls = [u for u in http_reqs if "/search-regions" in u or "/aura-field" in u]
            results.append(("no_search_backend", len(search_calls) == 0, search_calls[:3] if search_calls else "none"))
