#!/usr/bin/env python3
"""Smoke Phase 2.29: debug-only visible readiness indicator."""
    ...
    result = page.evaluate(
        """callHook => {
            ...
            return {
                ...
                indicatorVisible: Boolean(indicator && indicatorStyle && indicatorStyle.display !== "none"),
...
def main() -> int:
    ...
    indicator_text = debug_probe.get("indicatorText") or ""
    forbidden_terms = [
        '"features"',
        '"geometry"',
        '"coordinates"',
    ...
    checks = {
        ...
        "default_indicator_absent": default_probe["indicatorExists"] is False,
        "default_hook_absent_or_inert": default_probe["selfCheckHookExists"] is False,
        ...
        "debug_indicator_exists_and_is_visible": (
            debug_probe["indicatorExists"] is True
            and debug_probe["indicatorVisible"] is True
        ),
        "debug_indicator_marked_dev_debug": (
            "DEV DEBUG" in indicator_text
            and "readiness metadata" in indicator_text
        ),
        "debug_indicator_uses_sanitized_metadata_only": (
            "status:" in indicator_text
            and "passed:" in indicator_text
            and "substrate: legacy_search_regions" in indicator_text
            and "debug_only: true" in indicator_text
            and "default_behavior_changed: false" in indicator_text
            and "debug_indicator_visible: true" in indicator_text
            and "renderer_substrate_changed: false" in indicator_text
            and "aura_rendering_invoked: false" in indicator_text
            and "raw_payload_exposed: false" in indicator_text
            and "recommendation_surface_created: false" in indicator_text
            and "scoring_surface_created: false" in indicator_text
            and "final_truth_claimed: false" in indicator_text
            and not any(term in indicator_text.lower() for term in forbidden_terms)
        ),
        ...
        "indicator_causes_no_fetch_map_layer_or_registry_mutation": (
