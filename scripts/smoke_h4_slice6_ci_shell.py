#!/usr/bin/env python3
"""Static smoke: H4 Slice 6 — Comparison City Intelligence collapsible shell."""
...
def main() -> int:
    ...
    ci_fn = text.split("function renderComparisonCiSectionShellHtml", 1)[1].split("function renderComparisonCityBarHtml", 1)[0]
    ...
    check('data-ci-wired="false"' in ci_fn, "data-ci-wired=false marker")
    check('data-action="cmp-toggle-ci"' in ci_fn, "cmp-toggle-ci collapse action")
    check("ci-section-head" in ci_fn and "ci-grid" in ci_fn, "ci-section-head and ci-grid in shell")
    check("City Intelligence" in ci_fn, "City Intelligence title in shell")
    ...
    check("body.rm-beta-compare .ci-section.collapsed .ci-grid" in text, "CI collapse CSS")
    init_ws = text.split("function initComparisonWorkspace", 1)[1].split("let _screen5Token", 1)[0]
    check("[data-action='cmp-toggle-ci']" in init_ws, "CI toggle wired in initComparisonWorkspace")
    ...
    check('"city_intelligence"' in text.split("const CMP_WS_SECTIONS", 1)[1][:120], "city_intelligence in workspace sections")
