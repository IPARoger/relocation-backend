...
**Critical finding:** Active production UI **no longer hardcodes `http://127.0.0.1:8000`**. Zero matches in `map_CURRENT.html` or `app_shell.html`. Calls use same-origin relative paths (`API_BASE = ''`, `LIBRARY_API_BASE = ""`) and are served by `main_centerline_FIXER.py` (Web2 stack on port **8004**).

**Documentation drift:** `FEATURE_STATUS_BOARD.md` blocker **B-2**, `PRODUCTION_ACCEPTANCE_CHECKLIST.md` §3.3–3.4, and several architecture docs still describe five hardcoded port-8000 endpoints. That reflects a **pre-migration snapshot** (confirmed by `archives/validation_2026-05-15/html_snapshots/map_CURRENT.html`). B-2 should be reclassified after human review.

**Hardcoded port 8000 remains only in:** sandbox HTML, validation sandboxes, `Old File/`, archives, one dev JS bridge, and stale docs — classified **ARCHAEOLOGY** or **dev-only**.
...
All B-2-named endpoints exist on the same FastAPI app used for port 8004 smokes (`scripts/smoke_favorites.py`, `scripts/smoke_library_scaffold.py`, etc.).
...
|----------|--------|-------------------|---------|-------------------|----------------|-----------------------------------|--------|-------------|
| `/relocated-chart` | **MIGRATED** | `map_CURRENT.html` | 2125–2127 | `fetchRelocatedChart()` — map popup (custom click, city click, genie summary) | try/catch → fallback popup HTML + `console.error` | Popup relocated chart; city popup charts; genie popup summary; Screen 4/5 (via shell) | **S** (frontend done) | Update B-2 docs; smoke on 8004-only stack |
| `/relocated-chart` | **MIGRATED** | `app_shell.html` | 1555, 1642 | `hydrateRelocatedChart()` Screen 4; `hydrateComparisonColumns()` Screen 5 | try/catch → `warn-box` UI | Screen 4 chart facts; Screen 5 comparison columns (unblocks B-4 wiring) | **S** | Same as above |
| `/aspect-orb-at-point` | **PARTIAL** | `map_CURRENT.html` | 2473 | Custom-location popup, debug aura mode only (`debugAuraMode && isAuraPocOverlay`) | Silent skip if `!orbResp.ok`; outer try/catch on popup | Debug orb readout in popup (PoC overlay only) | **S** | Document PoC scope; optional UX error |
...
| `/aura-field` | **PARTIAL** | `map_CURRENT.html` | 3651, 4563 | `postAuraField()` → `renderAuraFieldProgressive()` (non-deliverable polygon path; warns to use raster) | Throws from `postAuraField` | Legacy polygon aura (superseded by raster) | **M** | Consider deprecating polygon path in UI |
| `/screen-pixel-truth` | **MIGRATED** | `map_CURRENT.html` | 5175, 5299, 5341, 5600 | `postScreenPixelTruth()` — debug `?screenPixelTruth=1` | Returns `{ response, body }`; caller checks status | Debug / QA pixel-truth grid only | **S** | None for production |
...
| `/search-regions` | **MIGRATED** | `map_CURRENT.html` | 4752 | Find Regions / genie engine (`${API_BASE}/search-regions`) | Part of genie render try/catch | Core Find Regions | **S** | Already on 8004 path via `API_BASE = ''` |

**Notes on PARTIAL:** Backend routes exist and are reachable on 8004, but `/aspect-orb-at-point` and aura endpoints enforce **Sun conjunct ASC PoC** scope (`is_aura_poc_overlay`, 400 on other overlays). That is reduced functionality vs a full overlay matrix, not a port migration gap.

---

## Hardcoded `127.0.0.1:8000` references (non-active)

Every `grep -rn "127.0.0.1:8000" --include="*.html" --include="*.js"` match outside active production files:

| Endpoint(s) used | File | Line | Status | Notes |
|------------------|------|------|--------|-------|
| `/search-regions` (+ sandboxes) | `map_SANDBOX_truth_reveal.html` | 178 | ARCHAEOLOGY | `const API = "http://127.0.0.1:8000"` |
...
**Completeness:** 25 repo-wide HTML/JS matches; 0 in active production UI; all non-archive active-tree matches accounted above (+ 2 archive snapshot + 6 Old File = full set).

---

## B-2 cross-check (`FEATURE_STATUS_BOARD.md`)

| B-2 claim | Inventory finding |
|-----------|-------------------|
| Five endpoints hardcode `http://127.0.0.1:8000` in `map_CURRENT.html` | **Stale.** Active file uses relative URLs; archived snapshot still shows hardcoded URLs. |
| Functions: `renderRasterAura()`, `renderAdaptiveAuraProgressive()`, `renderAuraField()`, `fetchRelocatedChart()`, `getAspectOrbAtPoint()` | Names mostly match (`renderRasterAuraProgressive`, `postAura*`, `fetchRelocatedChart`; orb inline in popup handler, not `getAspectOrbAtPoint`) |
...
| Comparison facts (B-4) | `/relocated-chart` | **No** (endpoint available) | Was mocked; `app_shell.html` now hydrates real charts — separate from port 8000 |
...
grep -rn "127.0.0.1:8000" --include="*.html" --include="*.js" . \
  | grep -v node_modules | grep -v backups | grep -v archives | grep -v ".tmp-chrome"
# → 17 matches (all sandbox/archaeology/dev)
